...
class ConfigLoader:
    ...
    def __init__(
        ...
        self.env = env
        config_path = config_path or os.getenv("CONFIG_PATH", "/opt/jobplat/config/settings.yaml")
    ...
    def load_env(self, env: str | None = None) -> EnvironmentConfig:
        ...
        if env_config.storage.type == "local":
            ...
            root_path = Path(os.getenv("DATA_ROOT", env_config.paths.root)).resolve()
            metadata_path = Path(os.getenv("METADATA_ROOT", env_config.paths.metadata)).resolve()
