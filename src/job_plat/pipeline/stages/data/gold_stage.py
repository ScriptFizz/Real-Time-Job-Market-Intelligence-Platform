from job_plat.context.contexts import GoldContext, StageExecutionContext
...
from job_plat.transformations.gold.v1_analytics.build_dimensions import (
    build_dim_jobs,
    build_dim_skills,
)
from job_plat.transformations.gold.v1_analytics.fact_job_skills import (
    build_fact_job_skills,
)
from job_plat.utils.helpers import StageSkip


class GoldStage(BaseStage[GoldContext, GoldOutputs]):
    STAGE_NAME = "gold"
    ...
    def __init__(
        ...
        super().__init__(
            datasets=datasets, partition_manager=partition_manager, ctx=gold_ctx
        )
    ...
    def transform(self, inputs: StageInputs) -> GoldOutputs:
        job_silver_df = inputs.get("job_silver_df")
        job_skills_silver_df = inputs.get("job_skills_silver_df")
        ...
        fact_df = build_fact_job_skills(
            job_skills_silver_df=job_skills_silver_df, dim_skills_df=dim_skills_df
        )

        return GoldOutputs(
            dim_jobs=dim_jobs_df, dim_skills=dim_skills_df, fact_job_skills=fact_df
        )

    def compute_metrics(self, outputs: GoldOutputs) -> Metrics:
    ...
    def evaluate_metrics(self, metrics: Metrics) -> None:
        ...
        if metrics.get("jobs") == 0:
            self.logger.error(
                "data_quality_issue", extra={"issue": "no_jobs_generated"}
            )
