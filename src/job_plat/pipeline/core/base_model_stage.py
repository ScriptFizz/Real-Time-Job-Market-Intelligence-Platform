...
class BaseModelStage(ABC):
    ...
    def read(self) -> dict:
        ...
        for name, dataset_cls in self.INPUT_MAP.items():
            ...
            time_window_column = ds.time_window_column
            
            if time_window_column is None:
                raise ValueError("No time window selection supported")
            
            df = spark.read.parquet(ds.path)
            
            df = df.filter(
                (col(time_window_column) >= start) &
                (col(time_window_column) < end)
            )
