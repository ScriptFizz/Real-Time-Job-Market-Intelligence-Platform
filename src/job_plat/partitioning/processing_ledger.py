...
class ProcessingLedger:
    ...
    def start_attempt(
        ...
        *,
        ...
        batch_id = self._build_batch_id(stage_name, partition_batch)
        ...
        self._insert_started_rows(source)
        return attempt

    def _insert_started_rows(self, source: DataFrame) -> None:
        target = DeltaTable.forPath(self.spark, self.path)
        (
            target.alias("target")
            .merge(source.alias("source"), self._attempt_match_condition())
    ...
    @staticmethod
    def _build_batch_id(stage_name: str, partitions: tuple[date, ...]) -> str:
        identity = ":".join(
            [stage_name, *(partition.isoformat() for partition in partitions)]
        )
        return hashlib.sha256(identity.encode("utf-8")).hexdigest()

    @staticmethod
    def _attempt_match_condition() -> Column:
        return (
            ...
            & (col("target.attempt_id") == col("source.attempt_id"))
        )
