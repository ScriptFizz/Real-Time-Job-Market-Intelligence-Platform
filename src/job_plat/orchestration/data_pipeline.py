from job_plat.context.contexts import BronzeContext, DataPipelineContext
...
def run_bronze_pipeline(
    ctx: BronzeContext, storage: Storage, connector: JobConnector
) -> None:
    stage = BronzeStage(bronze_ctx=ctx, storage=storage, connector=connector)
    stage.execute()
...
def run_silver_pipeline(
    ...
    stage = SilverStage(
        silver_ctx=ctx.silver, datasets=datasets, partition_manager=partition_manager
    )
...
def run_gold_pipeline(
    ...
    stage = GoldStage(
        gold_ctx=ctx.gold, datasets=datasets, partition_manager=partition_manager
    )
...
def run_data_pipeline(
    ...
    for connector in connectors:
        run_bronze_pipeline(ctx=ctx.bronze, storage=storage, connector=connector)
