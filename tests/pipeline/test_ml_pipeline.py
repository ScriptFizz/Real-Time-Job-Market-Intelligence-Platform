...
def test_ml_pipeline_orchestration(monkeypatch):
    ...
    monkeypatch.setattr(
        "job_plat.orchestration.ml_pipeline.run_feature_pipeline", feature_mock
    )

    monkeypatch.setattr("job_plat.orchestration.ml_pipeline.run_ml_pipeline", ml_mock)
    ...
    def feature_side_effect(*_args, **_kwargs):
        call_order.append("feature")

    def ml_side_effect(*_args, **_kwargs):
