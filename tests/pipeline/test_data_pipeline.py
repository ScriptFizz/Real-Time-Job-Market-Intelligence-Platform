...
def test_data_pipeline_orchestration(monkeypatch):
    ...
    monkeypatch.setattr(
        "job_plat.orchestration.data_pipeline.run_bronze_pipeline", bronze_mock
    )

    monkeypatch.setattr(
        "job_plat.orchestration.data_pipeline.run_silver_pipeline", silver_mock
    )

    monkeypatch.setattr(
        "job_plat.orchestration.data_pipeline.run_gold_pipeline", gold_mock
    )
    ...
    def silver_side_effect(*_args, **_kwargs):
        call_order.append("silver")

    def gold_side_effect(*_args, **_kwargs):
